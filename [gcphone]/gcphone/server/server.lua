...
function getNumberPhone(identifier)
    local result = exports.ghmattimysql:executeSync("SELECT vrp_user_identities.phone FROM vrp_user_identities WHERE vrp_user_identities.user_id = @identifier", {
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = exports.ghmattimysql:executeSync("SELECT vrp_user_identities.user_id FROM vrp_user_identities WHERE vrp_user_identities.phone = @phone_number", {
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
    ...
    if myPhoneNumber == '0' or myPhoneNumber == nil then
        ...
        until id == nil
        exports.ghmattimysql:execute("UPDATE vrp_user_identities SET phone = @myPhoneNumber WHERE user_id = @identifier", { 
...
function getContacts(identifier)
    local result = exports.ghmattimysql:executeSync("SELECT * FROM phone_users_contacts WHERE phone_users_contacts.identifier = @identifier", {
        ['@identifier'] = identifier
    })
    return result
end
...
function getMessages(identifier)
    local result = exports.ghmattimysql:executeSync("SELECT phone_messages.* FROM phone_messages LEFT JOIN vrp_user_identities ON vrp_user_identities.user_id = @identifier WHERE phone_messages.receiver = vrp_user_identities.phone", {
         ['@identifier'] = identifier
    })
    return result
...
function _internalAddMessage(transmitter, receiver, message, owner)
    ...
    local Query2 = "SELECT * FROM phone_messages ORDER BY id DESC LIMIT 1"
	local Parameters = {
    ...
    return exports.ghmattimysql:execute(Query .. Query2, Parameters)
    --]]
    ...
    local id = exports.ghmattimysql:executeSync("SELECT * FROM phone_messages ORDER BY id DESC LIMIT 1" , {})
    return id[1]
...
function getHistoriqueCall (num)
    local result = exports.ghmattimysql:executeSync("SELECT * FROM phone_calls WHERE phone_calls.owner = @num ORDER BY time DESC LIMIT 120", {
        ['@num'] = num
    })
    return result
end
    ...
    if is_valid == true then
        ...
        if vRP.getUserSource({destPlayer}) ~= nil then
            ...
            if srcTo ~= nil then
				...
				vRPclient.notify(srcTo, {"Primesti un apel !~n~Apasa pe ~y~L ~w~pentru a vedea apelul! ~n~Sageata dreapta - raspunde, ~y~Sageata stanga - respinge"})
            else
                --TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
            end
...
function getBourse()
    ...
    -- 
    -- local result = exports.ghmattimysql:executeSync("SELECT * FROM `recolt` LEFT JOIN `items` ON items.`id` = recolt.`treated_id` WHERE fluctuation = 1 ORDER BY price DESC",{})
    local result = {
        {
            libelle = 'Google',
            price = 125.2,
