...
local keyParam = Keys["F3"] -- e.g : Keys["H"] will be change the U key to the H key for lock/unlock a vehicle
...
AddEventHandler("ls:lock", function(lockStatus, vehicle)
	...
	if lockStatus == 1 or lockStatus == 0 then -- Si le véhicule est déverrouillé (on le verrouille):
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Vehicul ~r~Incuiat", 0.080)
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- Si le véhicule est verrouillé
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Vehicul ~g~Descuiat", 0.080)
		-- ## Notifications
...
AddEventHandler("ls:notify", function(text, time)
	...
	--Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, time)
