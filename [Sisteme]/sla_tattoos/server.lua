...
vRPclient = Tunnel.getInterface("vRP","sla_tattoos")
TSclient = Tunnel.getInterface("sla_tattoos","sla_tattoos")
...
local cfg = module("sla_tattoos", "cfg/tattoos")
  ...
  local tattoshop_choice = function(player,choice)
    ...
    if tattoo then
      ...
      print("tatto")
          ...
          vRPclient.notify(player,{"Ai platit ~r~R$ "..price..""})
        else
		...
		vRP.getUData({user_id,"vRP:tattoos",function(value)
          ...
          vRPclient.notify(player,{"~y~Aveți deja acest tatuaj"})
		  end
		end})
	  end
    end
