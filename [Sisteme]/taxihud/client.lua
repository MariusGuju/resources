...
local fareCost = 0.24 --(1.66 = $100 per minute) Cost per second
local costPerMile = 4
local initialFare = 20.0 -- the cost to start a fare
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          msg = "Apasa ~NUMPAD5~ pentru a porni taximetrul"
        ...
        local veh = GetVehiclePedIsIn(ped, false)
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          if IsControlJustReleased(0, 110)  then -- NUMPAD5
            TriggerEvent('taxi:toggleDisplay')
            Citizen.Wait(100)
          end
          if IsControlJustReleased(0, 108)  then -- NUMPAD4
            TriggerEvent('taxi:toggleHire')
            Citizen.Wait(100)
          end
          if IsControlJustReleased(0, 109) then -- NUMPAD6
...
if testMode then
  ...
  AddEventHandler('VehicleSpawn', function(vehicleName)
    ...
    local sawned_car = CreateVehicle(vehicle, coords, GetEntityHeading(myPed), true, false)
    SetVehicleOnGroundProperly(sawned_car)
    SetModelAsNoLongerNeeded(vehicle)
    Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(sawned_car))
  end)
