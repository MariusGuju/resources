...
function vRPclothes.getPlayerSkin()
    ...
    if thePlayer ~= nil then
        ...
        if user_id ~= nil then
            local result = exports.ghmattimysql:execute("SELECT * FROM vrp_users WHERE id = @user_id", {user_id = user_id})
            local skinu = nil
            ...
            if tonumber(askingPrice) then
                if tonumber(askingPrice) >= 0 then
                    ...
                    if (tonumber(vRP.getMoney({user_id})) >= askingPrice) then
                        ...
                        vRPclient.notify(thePlayer, {"Ai platit ~g~$"..askingPrice})
                        TriggerClientEvent("raid_clothes:hasEnough",thePlayer,menu)
                    else
                        vRPclient.notify(thePlayer, {"~r~Fonduri insuficiente!"})
                    end
                end
            end
        end
