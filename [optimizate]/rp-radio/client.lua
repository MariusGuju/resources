...
Radio.Labels = {        
	...
	{ "FRZL_RADIO_INPUT", "Introdu Frecventa" },
}
Radio.Commands = {
	{
		Enabled = true, -- Add a command to be able to open/close the radio
		Name = "radio", -- Command name
		Help = "Activeaza radioul portabil", -- Command help shown in chatbox when typing the command
		Params = {},
	...
	},
	{
		...
		Name = "Frecventa", -- Command name
		Help = "Schimba frecventa radio", -- Command help shown in chatbox when typing the command
		Params = {
			{name = "Numar", "Introdu Frecventa"}
		},
		Handler = function(src, args, raw)
			if Radio.Has then
				if args[1] then
	...
	while true do
		...
		if Radio.Open then
			...
			if not Radio.On then
				AddTextComponentSubstringPlayerName(Radio.Clicks and "~r~dezactiveaza~w~" or "~g~activeaza~w~")
			end
