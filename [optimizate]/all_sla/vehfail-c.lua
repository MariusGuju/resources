...
local damageFactorEngine = 3.0					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
local damageFactorBody = 3.0					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
...
local fixMessages = {
	"Ai pus dopul de la ulei la loc",
	"Ai oprit scurgerea de ulei folosind guma de mestecat",
	"Ai reparat tubul de ulei cu bandă adeziva",
	"Ai strans surubul vasului de ulei si ai oprit scurgerea",
	"Ai dat cu piciorul in motor si a revenit magic la viata",
	"Ai scos niste rugina din tubul scanteii",
	"Ai tipat la vehiculul tau si a avut cumva un efect"
}
local fixMessageCount = 7
local fixMessagePos = math.random(fixMessageCount)

local noFixMessages = {
	"Ai verificat dopul de ulei. Este inca acolo",
	"Te-ai uitat la motor, e la locul lui",
	"Te-ai asigurat ca banda de prindere inca tinea motorul la locul lui",
	"Ai marit volumul radioului. Doar a inecat zgomotele ciudate ale motorului",
	"Ati adaugat antifurt la tubul scanteii. Nu a facut nicio diferenta",
	"Nu repara niciodata ceva care nu este stricat, au spus ei. Nu ai ascultat. Cel putin nu s-a inrautatit"
}
...
AddEventHandler('iens:repair', function()
	if isPedInVehicle() then
		...
		if IsNearMechanic() then
			...
			notification("~g~Mecanicul ti-a reparat masina!")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. fixMessages[fixMessagePos] .. ", acum mergi de urgenta la un mecanic!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Masina ta este mult prea avariata.. Cheama un mecanic sa-ti ia masina cu platforma!")
			end
		else
		...
		notification("~y~Trebuie sa fii in masina pentru a o repara")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Nu ai permisiunea sa repari masini")
end)
