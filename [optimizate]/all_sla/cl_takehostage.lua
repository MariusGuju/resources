...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Ai nevoie de un pistol cu munitie pentru a lua jucatorul ca ostatic!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("Nu e nimeni prin preajma ca sa il poti sa il iei ostatic!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Apasa tasta [G] pentru al elibera, sau tasta [H] pentru al omora")
