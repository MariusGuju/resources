...
ctrl.getStates = function(windows)
    ...
    local inFront = (GetPedInVehicleSeat(veh,0) == ped)
    local states = {}
    tick = 0
...
ctrl.setSeat = function(d,s)
    ...
    if IsVehicleSeatFree(v,d.seat) then
        TaskWarpPedIntoVehicle(ped,v,d.seat)
    end
    lastData = gotOptions
    gotOptions = false
...
ctrl.setEngine = function(d,s)
    ...
    if fuel > 5 then
        local running = Citizen.InvokeNative(0xAE31E7DF9B5B132E,v) -- GetIsVehicleEngineRunning
        SetVehicleEngineOn(v,not running,true,true)
        if running then
