...
AddEventHandler("smartweather:toggleWeather",function(from)
	...
	TriggerClientEvent("mesajPeChat", -1, "SmartWeather",{0,0,0},message)
end)

function handleAdminCheck(from)
	if( adminOnlyPlugin and not(isAdmin(getIdentifier(from, "steam"))) )then
		TriggerClientEvent('mesajPeChat', from, "SmartWeather", {200,0,0} , "You must be an admin to use this command.")
		return false
	end
	return true
end
...
AddEventHandler('mesajPeChat', function(from,name,message)
	if(string.sub(message,1,1) == "/") then
		...
		if(cmd == "/setweather")then
			...
			if(wtype == nil)then
				TriggerClientEvent('mesajPeChat', from, "SmartWeather", {200,0,0} , "Usage: /setweather CLEAR")
				return
			end

			if(weatherTree[wtype] == nil)then
				TriggerClientEvent('mesajPeChat', from, "SmartWeather", {200,0,0} , "Invalid weather type, valid weather types below")
				TriggerClientEvent('mesajPeChat', from, "", {255,255,255} , table.concat(getTableKeys(weatherTree)," "))
				return
			end
			...
			TriggerClientEvent("mesajPeChat", -1, "SmartWeather", {200,0,0}, name.." has updated the weather to: "..wtype) -- Ingame
		end
	end
