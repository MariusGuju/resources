...
Citizen.CreateThread(function()
	while true do
		...
		if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, -1212.3618, -1576.8405, 4.1115) < 10.0)then
			DrawText3D(-1212.3618, -1576.8405, 4.1115+0.4, "~y~Flexare", 1.2)
		...
		if(theGym ~= nil)then
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, theGym[1], theGym[2], theGym[3]) < 10.0)then
				DrawText3D(theGym[1], theGym[2], theGym[3]+0.4, "~y~Magazin", 1.2)
				...
				if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, theGym[1], theGym[2], theGym[3]) < 1.0)then
					incircle = true
					if(incircle == true) then
						gym_DisplayHelpText("Apasa ~INPUT_CONTEXT~ pentru a accesa ~g~Magazinul")
						if(IsControlJustReleased(1, 51))then
							inGymCircle = true
								...
								if(IsControlJustReleased(1, 51))then
									vRPserver.hasMembership({}, function(membership)
										if(membership)then
											...
											vRP.notify({"~w~[GYM] ~r~Nu ai abonament!"})
										end
									end)
								end
							end
...
Citizen.CreateThread(function()
	while true do
		...
		if(inWorkout) and (workDone < 5)then
			workDone = workDone + 1
			strenght = randomFloat(0.79, 1.4)
			vRPserver.gainStrenght({strenght})
			vRP.notify({"~g~Simti cum ti se umfla muschii iar bratele cum iti obosesc!"})
		end
	end
end)
