...
AddEventHandler("lscustom:sendDB", function(upgrades, vehicle)
	exports.ghmattimysql:execute("UPDATE vrp_user_vehicles SET upgrades = @upgrades WHERE user_id = @user_id AND vehicle = @vehicle", {upgrades = upgrades, user_id = vRP.getUserId({source}), vehicle = vehicle}, function()end)
end)