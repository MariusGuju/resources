...
vRPserver = Tunnel.getInterface("vrp","vrp_showromv2")
vRPshowroom = Tunnel.getInterface("vrp_showromv2","vrp_showromv2")
vRPshowroomC = {}
Tunnel.bindInterface("vrp_showromv2",vRPshowroomC)
vRPserver = Tunnel.getInterface("vrp_showromv2","vrp_showromv2")

local coordonateOpenShowroom = {-29.795457839966,-1105.2568359375,26.422372817993}
    ...
    if inShowroom == true then
        ...
        CreateThread(function()
            while true do
                Wait(1000)
                timp = timp - 1
                if timp == 0 or IsControlJustPressed(0,23) then
                    rablactuala = GetVehiclePedIsUsing(GetPlayerPed(-1))
                    DeleteEntity(rablactuala)
                    for k in pairs(categoriile) do
                        categoriile[k] = nil
