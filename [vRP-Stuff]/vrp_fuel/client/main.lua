		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				elseif IsFueling then
					...
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Apasa ~g~G ~w~pentru a opri ~g~$" .. price .. " ~w~+  taxe")
					...
					if IsControlJustReleased(0, 47) then
						...
						TriggerServerEvent('LegacyFuel:PayFuSamibagel', price)
						Citizen.Wait(2500)
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Acest vehicul are prea multa benzina")
					SetVehicleDoorsLockedForAllPlayers(vehicle, false)
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Nu ai destui bani pentru a da refill")
					SetVehicleDoorsLockedForAllPlayers(vehicle, false)
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Apasa ~g~G ~w~ pentru a baga benzina. ~g~$1/~w~L + taxe")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Apasa ~g~G ~w~pentru a opri alimentarea.  " .. fuel .. "% - Canistra: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Apasa ~g~G ~w~pentru a umple vehiculul cu canistra")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsFueling then
			...
			local fuelthis = integer / 20
			local newfuel  = fuel + fuelthis

			price = price + fuelthis * 0.2
	...
	while true do
		...
		if vehicle and engine then
			...
			if rpm > 0.9 then
				...
				Citizen.Wait(1500)
			elseif rpm > 0.8 then
				rpmfuelusage = fuel - rpm / 3
				Citizen.Wait(2500)
			elseif rpm > 0.7 then
				rpmfuelusage = fuel - rpm / 4
				Citizen.Wait(3500)
			elseif rpm > 0.6 then
				rpmfuelusage = fuel - rpm / 5
				Citizen.Wait(4500)
			elseif rpm > 0.5 then
				rpmfuelusage = fuel - rpm / 6
				Citizen.Wait(5500)
			elseif rpm > 0.4 then
				rpmfuelusage = fuel - rpm / 7
				Citizen.Wait(6500)
			elseif rpm > 0.3 then
				rpmfuelusage = fuel - rpm / 8
				Citizen.Wait(7500)
			elseif rpm > 0.2 then
				rpmfuelusage = fuel - rpm / 9
				Citizen.Wait(9500)
			else
				rpmfuelusage = fuel - rpm / 10
				Citizen.Wait(15000)
			end
...
local gas_stations = {
	...
	{ ['x'] = -62.688629150391, ['y'] = -2533.4704589844, ['z'] = 6.1358118057251} 
}
