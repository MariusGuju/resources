...
local function DriveInGarage()
	...
	--	LSCMenu:addPurchase("Repair vehicle",round(250+150*damage,0), "Full body repair and engine service.")
		...
		AddMod(21,LSCMenu.categories.Lights,"HEADLIGHTSCOLOR", "HEADLIGHTSCOLOR", nil, false)
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		if ok ~= null then
			...
			local faruriColor = GetVehicleXenonLightsColour(veh)
			local vmod = ""
			for i = 0, 48 do
			...
			TriggerServerEvent("lscustom:sendDB",ups, name, faruriColor)
		end
						...
						if LSC_Config.oldenter then			
							...
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												...
												drawTxt("Apasa ~b~ENTER~w~ pentru a modifica vehiculul",0,1,0.5,0.8,0.3,255,255,255,255)
											end
										else
									...
									drawTxt("~r~Masina nu a fost modificata",0,1,0.5,0.8,0.3,255,255,255,255)
								end
							end
						else
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											...
											drawTxt("~r~Asteapta, este deja cineva",0,1,0.5,0.8,0.3,255,255,255,255)
										end
									else
										inside = true
										currentpos = pos
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "headlightscolor" then
