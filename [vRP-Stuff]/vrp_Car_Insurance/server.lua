...
RegisterServerEvent("insurance:cumsucra")
AddEventHandler("insurance:cumsucra", function()
	local user_id = vRP.getUserId({source})
	local player = vRP.getUserSource({user_id})
	if vRP.tryPayment({user_id,150000}) then
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"police.askasigurare"}) then
