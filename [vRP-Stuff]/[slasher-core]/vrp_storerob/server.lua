...
vRPclient = Tunnel.getInterface("vRP","vrp_storerob")

local banks = {
	["Lindsay Circus"] = {
		position = { ['x'] = -705.94110107422, ['y'] = -915.48120117188, ['z'] = 18.215589523315 },
		reward = 100000 + math.random(800,1500),
...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('chatMessage', -1, '[ ProTV ]', {255, 0, 0}, "Jaful a fost anulat la : ^2" .. banks[robb].nameofbank)
	end
end)
...
AddEventHandler('es_bank:rob', function(robb)
  ...
  if vRP.hasPermission({user_id,"cop.whitelisted"}) then
    vRPclient.notify(player,{"~r~Politistii nu pot jefui magazine."})
  else
    if #cops >= 2 then -- change 2 to the minimum amount online necessary
	  if banks[robb] then
		  ...
		  if (os.time() - bank.lastrobbed) < 600 and bank.lastrobbed ~= 0 then
			  TriggerClientEvent('chatMessage', player, 'ROBBERY', {255, 0, 0}, "Magazinul asta a fost deja jefuit. Trebuie sa mai astepti: ^2" .. (1200 - (os.time() - bank.lastrobbed)) .. "^0 secunde.")
			  return
		  end
		  TriggerClientEvent('chatMessage', -1, '[ ProTV ]', {255, 0, 0}, "Jaf in desfasurare la: ^2" .. bank.nameofbank.. "^4Civilii sunt rugati sa stea departe de acest magazin!")
		  TriggerClientEvent('chatMessage', player, 'Sistem~Jaf', {255, 0, 0}, "Ai dat un jaf la : ^2" .. bank.nameofbank .. "^0, nu pleca prea departe de magazin!")
		  TriggerClientEvent('chatMessage', player, 'Sistem~Jaf', {255, 0, 0}, "Stai ascuns ^1 6 ^0minute si banii sunt ai tai! Fugi sa nu te prinda!")
		  TriggerClientEvent('es_bank:currentlyrobbing', player, robb)
		  TriggerClientEvent('es_bank:bagablip', source)
		  ...
		  SetTimeout(360000, function()
			  if(robbers[savedSource])then
				  if(user_id)then
					  ...
					  TriggerClientEvent('chatMessage', -1, '[ ProTV ]', {255, 0, 0}, "Jaful s-a terminat la : ^2" .. bank.nameofbank .. "^0!")
