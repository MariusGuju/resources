...
AddEventHandler('es_bank:toofarlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'Sistem~Jaf', {255, 0, 0}, "Jaful a fost anulat! NU vei primi nimic!")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_bank:playerdiedlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'Sistem~Jaf', {255, 0, 0}, "Jaful a fost anulat, ai murit!")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
RegisterNetEvent('es_bank:bagablip')
AddEventHandler('es_bank:bagablip', function(robb)
...
AddEventHandler('es_bank:robberycomplete', function(reward)
	robbing = false
	TriggerEvent('chatMessage', 'Sistem~Jaf', {255, 0, 0}, "Jaful s-a terminat, ai primit:^2" .. reward.. " $")
	bank = ""
	secondsRemaining = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(banks)do
		...
		AddTextComponentString("Jefuieste Magazinul")
...
Citizen.CreateThread(function()
	while true do
				...
				if not robbing then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2)then
						if (incircle == false) then
							bank_DisplayHelpText("Apasa ~INPUT_CONTEXT~ pentru a da jaf!~b~" .. v.nameofbank .. "~w~ Politia va fi anuntata!")
						end
						incircle = true
		...
		if robbing then
			...
			bank_drawTxt(0.66, 1.44, 1.0,1.0,0.4, "Jefuiesti magazinul! Timp ramas: ~r~" .. secondsRemaining .. "~w~ seconds", 255, 255, 255, 255)
			...
			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > 10)then
