...
Mai du-te-n ma-ta ca l-am editat.
]]
...
--Setup main menu
local LSCMenu = Menu.new("Los Santos Customs","CATEGORIES", 0.86,0.13,0.24,0.36,0,{255,255,255,255})
...
local function AddMod(mod,parent,header,name,info,stock)
	...
	if (GetNumVehicleMods(veh,mod) ~= nil and GetNumVehicleMods(veh,mod) > 0) or mod == 18 or mod == 22 or mod == 21 then
	...
	if DoesEntityExist(veh) then
		...
		elseif type(LSC_Config.menu.position) == 'string' then
			if LSC_Config.menu.position == "left" then
				LSCMenu.config.position = { x = 0.77, y = 0.13}
			elseif  LSC_Config.menu.position == "right" then
				LSCMenu.config.position = { x = 1-0.77, y = 0.13}
			end
		end
		...
		--myveh.vstancer = exports["vstancer"]:GetVstancerPreset(veh)
		myveh.mods = {}
		for i = 0, 48 do
			myveh.mods[i] = {mod = nil}
		end
		for i,t in pairs(myveh.mods) do 
			if i == 22 or i == 18 or i == 21 then
		...
		AddMod(21,LSCMenu.categories.Lights,"XENONHEADLIGHTS", "XENONHEADLIGHTS", nil, false)
		...
		LSCMenu.categories:addSubMenu("WHEELS", "Wheels", "Custom rims, tires and colors.",true)
			--wcamber = LSCMenu.categories.Wheels:addSubMenu("CAMBER", "CAMBERS", "Custom rims in all styles and sizes.",true)
			--	if IsThisModelABike(GetEntityModel(veh))then
			--		vRP.notify({"~r~Nu ptoti modifica o motocicleta."})
			--	else
			--		fwheelsc = wcamber:addSubMenu("Rotile din Fata", "Seteaza camberul pe rotile din fata.", nil,true)
			--		local btn = fwheelsc:addPurchase("[+] Creste Camber",10)
			--	end
			...
			if not IsThisModelABike(GetEntityModel(veh)) then
				stn = LSCMenu.categories.Wheels:addSubMenu("STANCER", "Stancer Menu", "",true)
				cambers = stn:addSubMenu("Camber", "Camber", nil,true)
				for n, camber in pairs(LSC_Config.prices.camber) do
					btn = cambers:addPurchase(camber.name,camber.price)btn.colorindex = camber.colorindex btn.reset = camber.reset
				end
	
				dist = stn:addSubMenu("Distantiere", "Distantiere", nil,true)
				for n, dsc in pairs(LSC_Config.prices.distantiere) do
					btn = dist:addPurchase(dsc.name,dsc.price)btn.colorindex = dsc.colorindex btn.reset = dsc.reset
				end
			end
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		if ok ~= null then
			...
			--local xenonHeadlights = GetVehicleXenonLightsColour(veh)
			--local vmod = ""
			--for i = 0, 48 do
			...
			--	end
			--end
			--local stancerdata = exports["vstancer"]:GetVstancerPreset(veh)
			local vstancer1 = stancerdata[1]
			local vstancer2 = stancerdata[2]
			local vstancer3 = stancerdata[3]
			local vstancer4 = stancerdata[4]
			...
			print("[Tunning] WheelType = "..wheeltype)
			...
			local mods47 = GetVehicleMod(veh, 47)
			...
			local mods26 = GetVehicleMod(veh, 26)
			...
			--print(mods48,mods47,mods45,mods44,mods43,mods42,mods41,mods40,mods39,mods38,mods37,mods36,mods35,mods34,mods33,mods32,mods31,mods30,mods29,mods28,mods27,mods26,mods25,mods24,mods23,mods16,mods15,mods14,mods13,mods12,mods11,mods10,mods9,mods8,mods7,mods6,mods5,mods4,mods3,mods2,mods1,mods0)
			...
			TriggerServerEvent("lscustom:sendDB",
				plateindex, 
				primarycolor, 
				secondarycolor, 
				pearlescentcolor, 
			...
			--TriggerServerEvent("lscustom:sendDB",plateindex,primarycolor,secondarycolor,pearlescentcolor,wheelcolor,lightcolor1,neoncolor1,neoncolor2,neoncolor3,windowtint,wheeltype,mods0,mods1,mods2,mods3,mods4,mods5,mods6,mods7,mods8,mods9,mods10,mods11,mods12,mods13,mods14,mods15,mods16,turbo,tiresmoke,xenon,mods23,mods24,neon0,neon1,neon2,neon3,bulletproof,smokecolor1,smokecolor2,smokecolor3,variation, name,vehicleplate)
		end
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif button.modtype and button.mod then
		if button.modtype ~= 18 and button.modtype ~= 22 and button.modtype ~= 21 then
		...
		elseif button.modtype == 22 then
			--print(button.modtype,button.mod)
			ToggleVehicleMod(veh,button.modtype, button.mod)
		elseif button.modtype == 21 then
			ToggleVehicleMod(veh,button.modtype, button.mod)
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "xenonheadlights" then
		if button.purchased or CanPurchase(price, canpurchase) then
			myveh.mods[button.modtype].mod = button.mod
			ToggleVehicleMod(veh,button.modtype,22)
			SetVehicleXenonLightsColour(veh, button.mod)
		end
	...
	elseif mname == "sport" or mname == "muscle" or mname == "lowrider" or mname == "back wheel" or mname == "front wheel" or mname == "highend" or mname == "suv" or mname == "offroad" or mname == "tuner" then
		if button.purchased or CanPurchase(price, canpurchase) then
			...
			SetVehicleWheelType(veh,button.wtype)
			SetVehicleMod(veh,button.modtype,button.mod)
			print(button.modtype,button.mod,button.wtype)
		end
	...
	elseif mname == "camber" then
		if button then
			UnfakeVeh()
			--local preset = exports["vstancer"]:GetVstancerPreset(veh)
			--if CanPurchase(price, canpurchase) then
				if not button.reset then
					exports["vstancer"]:SetVstancerPreset(veh,tonumber(preset[1]),tonumber(preset[2]+button.colorindex),tonumber(preset[3]),tonumber(preset[4]+button.colorindex))
				else
					exports["vstancer"]:SetVstancerPreset(veh,tonumber(preset[1]),myveh.vstancer[2],tonumber(preset[3]),myveh.vstancer[4])
				end
			--else
			--	LSCMenu:showNotification("Nu ai suficienti bani!")
			--end
		end
	elseif mname == "distantiere" then
		if button then
			UnfakeVeh()
			--local preset = exports["vstancer"]:GetVstancerPreset(veh)
			--if CanPurchase(price,canpurchase)then
				if not button.reset then
					exports["vstancer"]:SetVstancerPreset(veh,tonumber(preset[1]+button.colorindex),tonumber(preset[2]),tonumber(preset[3]+button.colorindex),tonumber(preset[4]))
				else
					exports["vstancer"]:SetVstancerPreset(veh,myveh.vstancer[1],tonumber(preset[2]),myveh.vstancer[3],tonumber(preset[4]))
				end
			--else
			--	LSCMenu:showNotification("Nu ai suficienti bani!")
			--end
...
RegisterCommand("fixx",function(source)
	AddBlips()
	...
	--local stancerdata = exports["vstancer"]:GetVstancerPreset(veh)
	print(stancerdata[1],stancerdata[2],stancerdata[3],stancerdata[4])
end)