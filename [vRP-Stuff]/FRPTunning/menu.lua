		...
		while self.visible do
			...
			ty = y - height/2.8
				...
				for i,btn in pairs(buttons) do
					if i >= menu.from and i <= menu.to then
						if menu.selected == i then
							...
							DrawRect(x,  y, width, height,3, 186, 252,cfg.sbg_color.a)
							...
							elseif btn.price ~= nil then	
								if btn.price == 0 then
									...
									if not btn.purchased then
										drawRightMenuText("$ "..formatMoney(tonumber(btn.price)),cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
									else
							...
							elseif btn.price ~= nil then
								if btn.price ~= 0 then
									if not btn.purchased then
										drawRightMenuText("$ "..formatMoney(tonumber(btn.price)),cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
									else
				...
				if self.notification ~= nil then
					DrawRect(x,  y-height + y-ty + 0.0055, width, 0.0035,3, 186, 252,255)
