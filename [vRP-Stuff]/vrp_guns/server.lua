...
function vRPweap.infoweap(source)
	...
	local player = vRP.getUserSource({user_id})
     exports.ghmattimysql:executeSync("SELECT * FROM vrp_arme WHERE user_id = @user_id", {['@user_id'] = user_id}, function (weapinfo)
        if #weapinfo > 0 then 
                                ...
                                if vRP.tryFullPayment({user_id,pretul}) then
                                    exports.ghmattimysql:execute("SELECT weapon FROM vrp_arme WHERE user_id = @user_id AND weapon = @weapon", {['@user_id'] = user_id, ['@weapon'] = armaNume}, function (rows)
                                        if #rows > 0 then
                                            ...
                                            exports.ghmattimysql:executeSync("INSERT INTO vrp_arme (user_id,clasa,weapon,hash,gloante,componente,inventar,poateSaVanda) VALUES (@user_id,@clasa,@weapon,@hash,@gloante,@componente,@inventar,@poateSaVanda)", {
                                                ['@user_id'] = user_id, 
                                                ['@clasa'] = clasa,
                                                ['@weapon'] = armaNume,
                                                ['@hash'] = armaModel,
        ...
        if #rows > 0 then
          if rows[1].componente ~= nil then
              ...
              if componenteleActuale == 0 then
                  ...
                  exports.ghmattimysql:executeSync("UPDATE vrp_arme SET componente = @componente WHERE user_id = @user_id AND hash = @hash", {
                      ['@user_id'] = user_id, 
                      ['@hash'] = armaModel,
                      ['@componente'] = compUpd
                  }, function (rows)
...
AddEventHandler('updateArma',function(armaNabHash,gloanteUpdate)
	local user_id = vRP.getUserId({source})
    local player = vRP.getUserSource({user_id})
    exports.ghmattimysql:executeSync("UPDATE vrp_arme SET gloante = @gloante WHERE user_id = @user_id AND hash = @hash", {
        ['@user_id'] = user_id, 
        ['@hash'] = armaNabHash,
