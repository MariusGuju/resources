...
RegisterNUICallback("menu",function(data,cb)
  ...
  elseif data.act == "valid" then
    ...
    print('vRP-Client-GUI.lua ['..data.id,data.chice,data.mod..']')
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
	if string.len(data.result) <= 150 then
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and not tastaBlocata and (not tvRP.isInComa() or not cfg.coma_disable_menu) and (not tvRP.isHandcuffed() or not cfg.handcuff_disable_menu) then 
