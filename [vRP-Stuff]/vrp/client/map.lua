...
eskInfoNotification = {}
function tvRP.eskNotify(text,ms)
    print(text,ms)
    if text and ms then
      table.insert(eskInfoNotification, { ["text"] = text, ["time"] = ms, ["startTime"] = GetGameTimer() })
    end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if #eskInfoNotification > 0 then
            for notificationIndex = 1, #eskInfoNotification do
                local notification = eskInfoNotification[notificationIndex]
                ...
                if notification then
                    ...
                    smek = notificationIndex - 0.2

                    eskDrawText(1.200, 1.285 - (notificationIndex * 0.05), 1.0,1.0,0.50, notification["text"],notification["opacity"], 255, 255, 255, 255)

                    if notificationTimer >= 100 then
                        table.remove(eskInfoNotification, notificationIndex)
                    end
                end
            end
        end
...
function eskDrawText(x,y ,width,height,scale, text,opacity, r,g,b,a, outline)
    if opacity > 255 then
        opacity = 255
