...
function tvRP.spawnGarageVehicle(vtype,name,damage,veh_plate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
  ...
  if vehicle == nil then
    ...
    if HasModelLoaded(mhash) then
      ...
      if damage == "Da" then
        SetVehicleEngineHealth(nveh, 0)
        SmashVehicleWindow(nveh, 0)
        SmashVehicleWindow(nveh, 1)
        SmashVehicleWindow(nveh, 2)
        SmashVehicleWindow(nveh, 3)
        SmashVehicleWindow(nveh, 4)
        SetVehicleTyreBurst(nveh, 1, true)
      elseif damage == "Nu" then
        SetVehicleEngineHealth(nveh, 1000.0)
      end
    end
  else
    tvRP.notify("Poti avea numai un vehicul afara din acest garaj",2000,4)
  end
end
...
function tvRP.spawnGarageVehicleAdmin(name,veh_plate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
...
function tvRP.vc_toggleLock(vtype)
  ...
  if vehicle then
    ...
    if locked then
      ...
      tvRP.notify("Masina descuiata",4000,1)
    else
      SetVehicleDoorsLocked(veh,2)
      SetVehicleDoorsLockedForAllPlayers(veh, true)
      tvRP.notify("Masina incuiata",4000,2)
    end
  end
end
