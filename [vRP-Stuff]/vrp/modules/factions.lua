...
function getFactionMembers()
	for i, v in pairs(factions) do
		--exports.ghmattimysql:execute("vRP/get_faction_members", {faction = tostring(i)}, function(rows, affected)
		exports.ghmattimysql:execute("SELECT * FROM vrp_users WHERE faction = @faction", {['@faction'] = tostring(i)}, function(rows)
...
function vRP.setFactionLeader(user_id)
	...
	if tmp then
		tmp.fLeader = 1
		exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionLeader = @leader WHERE id = @user_id", {['@user_id'] = user_id, ['@leader'] = 1})
	end
end
...
function vRP.setFactionNonLeader(user_id)
	...
	if tmp then
		tmp.fLeader = 0
		exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionLeader = @leader WHERE id = @user_id", {['@user_id'] = user_id, ['@leader'] = 0})
	end
end
...
function vRP.setFactionCoLeader(user_id)
	...
	if tmp then
		tmp.fCoLeader = 1
		exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionCoLeader = @coleader WHERE id = @user_id", {['@user_id'] = user_id, ['@coleader'] = 1})
	end
end
...
function vRP.setFactionNonCoLeader(user_id)
	...
	if tmp then
		tmp.fCoLeader = 0
		exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionCoLeader = @coleader WHERE id = @user_id", {['@user_id'] = user_id, ['@coleader'] = 0})
	end
end
	...
	for i, v in pairs(ranks) do
		...
		if(rankTitle == rankName)then
			if(i == #ranks)then
				...
				tmp.fRank = theRank
				exports.ghmattimysql:execute("UPDATE vrp_users SET factionRank = @rank WHERE id = @user_id", {['@user_id'] = user_id, ['@rank'] = theRank})
				return true
			end
		end
	end
	...
	for i, v in pairs(ranks) do
		...
		if(rankTitle == rankName)then
			if(i == 1)then
				...
				local theRank = tostring(ranks[i-1].rank)
				tmp.fRank = theRank
				exports.ghmattimysql:execute("UPDATE vrp_users SET factionRank = @rank WHERE id = @user_id", {['@user_id'] = user_id, ['@rank'] = tostring(ranks[i-1])})
				return true
			end
		end
	end
	...
	if (player) then
		...
		if ngroup then
			...
			if tmp then
				...
				tmp.fLeader = 0
				tmp.fCoLeader = 0
				exports.ghmattimysql:execute("UPDATE vrp_users SET faction = @group, factionRank = @rank WHERE id = @user_id", {['@user_id'] = user_id, ['@group'] = theGroup, ['@rank'] = factionRank})
			...
			vRP.addUserGroup(user_id,factionRank)
		end
	end
end
		...
		if tmp then
			for i, v in pairs(factionMembers[theGroup])do
				if (v.id == user_id) then
					...
					tmp.fLeader = 0
					tmp.fCoLeader = 0
					exports.ghmattimysql:execute("UPDATE vrp_users SET faction = @group, factionRank = @rank WHERE id = @user_id", {['@user_id'] = user_id, ['@group'] = "user", ['@rank'] = "none"})
					exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionLeader = @leader WHERE id = @user_id", {['@user_id'] = user_id, ['@leader'] = 0})
					exports.ghmattimysql:execute("UPDATE vrp_users SET isFactionCoLeader = @coleader WHERE id = @user_id", {['@user_id'] = user_id, ['@coleader'] = 0})
...
local function ch_inviteFaction(player,choice)
	...
	local factionRank = factions[theFaction].fRanks
	...
	SetTimeout(500, function()
		vRP.buildMenu("Rankuri & Salarii", {player = player}, function(rsMenu)
			...
			for i, v in pairs(ranks) do
				...
				rsMenu["["..i.."] "..facRank] = {ch_dummySalary, "Salariu: <font color='green'>"..salary.." (de) euro.</font>"}
			end
			...
			if(vRP.hasUserFaction(user_id))then
				...
				choices["Meniu Factiune"] = {function(player,choice)
					vRP.buildMenu(theFaction, {player = player}, function(menu)
						...
						if(fType == "M.A.I") or (fType == "U.P.U")then
							if(vRP.hasGroup(user_id,"onduty"))then
...
vRP.registerMenuBuilder("admin", function(add, data)
	...
	if user_id ~= nil then
		local choices = {}
		if vRP.hasPermission(user_id,"add.faction") then
	...
	if(vRP.hasUserFaction(user_id))then
		...
		if(args[1] == nil)then
			...
			for i, v in ipairs(fMembers) do
				...
				TriggerClientEvent('chatMessage', player, "^5["..theFaction.."] ^7| " .. vRP.getPlayerName(source) .. ": " ..  rawCommand:sub(3))
			end
		end
	end
end)
