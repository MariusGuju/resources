          ...
          if in_coma then
            if vRP.tryGetInventoryItem(user_id,"medkit",1,true) then
              ...
              if vRP.tryFullPayment(nplayer,amount) then
                 vRPclient.notify(player,{"Jucatorul ~p~"..GetPlayerName(nplayer).."~w~ a platit ~g~$50~w~ pentru reteta si au intrat in contul Primariei!"})
                 vRPclient.notify(nplayer,{"Ai platit ~g~ $50~w~ pentru retata  mirobolanta de la medicul ~r~"..GetPlayerName(player)})
                 
              else 
                 vRPclient.notify(player,{"Jucatorul ~p~"..GetPlayerName(nplayer).."~w~ E sarac, n-are ~g~ 50$ pentru retata!"})
              end
            end
          else
