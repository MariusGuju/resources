...
function vRP.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service then
    ...
    for k,v in pairs(players) do
      ...
      -- call request
      if sender ~= nil then
        vRP.request(v,lang.phone.service.ask_call({service_name, "["..vRP.getUserId(sender).."] "..GetPlayerName(sender) ,htmlEntities.encode(msg)}), 30, function(v,ok)
...
local function ch_announce_alert(player,choice) -- alert a announce
  ...
  local user_id = vRP.getUserId(player)
  local playyer = vRP.getUserSource(user_id)
  local playyer_name = GetPlayerName(playyer)
