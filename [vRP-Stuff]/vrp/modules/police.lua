...
local choice_handcuff = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
      if nuser_id == 1 then
        vRPclient.notify(player,{"Nu poti sa-i pui catuse lui eskape"})
      else
          ...
          if handcuffed then
            vRPclient.getWeapons(nplayer,{},function(weapons)
              for k,v in pairs(weapons) do -- display seized weapons
                ...
                vRPclient.notify(player,{"Ai confiscat armele unui jucator"})
                if v.ammo > 0 then
...
local choice_store_weapons = {function(player, choice)
  local user_id = vRP.getUserId(player)
	...
	vRPclient.getWeapons(player,{},function(weapons)
		vRP.request(player,"Esti sigur ca vrei sa strangi armele?",30,function(player,ok)
			if ok then 
			  ...
			  vRPclient.notify(player,{"~g~Arme stranse"})
				SetTimeout(10000,function()
					isStoring = false
				end)
			  else
				vRPclient.notify(player,{"~o~Ai strans deja armele."})
			  end
			end
		end)
	end)
...
vRP.registerMenuBuilder("main", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission(user_id, "traficant.acces") then
