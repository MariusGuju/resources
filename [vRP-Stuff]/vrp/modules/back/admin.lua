...
local function ch_addgroup(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.group.add") and onduty[usrID] == 0 then
...
local function ch_removegroup(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.group.remove") and onduty[usrID] == 0 then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
...
local function ch_kick(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.kick") and onduty[usrID] == 0 then
...
local function ch_unban(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.unban") and onduty[usrID] == 0 then
...
local function ch_tptome(player,choice)
  if user_id ~= nil and onduty[usrID] == 0 then
  vRPclient.getPosition(player,{},function(x,y,z)
