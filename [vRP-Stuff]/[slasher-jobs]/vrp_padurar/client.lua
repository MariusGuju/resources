...
Citizen.CreateThread(function()
    while true do 
        ...
        for k, v in pairs(takejob) do
            ...
            if dis < 1 then
                Draw3DText(PC.x, PC.y, PC.z, "~h~~c~Apasa ~g~'SpaceBar' ~c~sa te angajezi ! ~g~(Padurar)", 0.5)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(forestcar) do 
            local dis = GetDistanceBetweenCoords(PC.x, PC.y, PC.z, v[1], v[2], v[3], true)
            if dis < 5 then 
            Draw3DText(PC.x, PC.y, PC.z, "~h~~c~Apasa ~g~'SpaceBar' ~c~sa iti iei masina de ~g~Padurar", 0.5)
            DrawMarker(1, v[1],v[2],v[3] - 1 , 0, 0, 0, 0, 0, 0, 1.5001,1.5001,0.5001, 255,255,255, 200, 0, 0, 0, 1, 0, 0, 0)
            if dis < 1 then
                if IsControlJustPressed(0, 203) then
