			...
			if(theTrailer ~= nil)then
				if not (IsEntityDead(theTrailer)) then
					...
					if (ok) then
						...
						trucker_drawTxt("~y~[TRUCKER] ~r~Ai nevoie de ~y~TIR ~r~si ~y~REMORCA ~r~pentru a finaliza livrarea!",1,1,0.5,0.9,0.8,255,255,255,255)
					end
					...
					elseif(isInVehicle == true)then
						if not IsEntityAttached(theTrailer) then
							...
							trucker_drawTxt("~g~Destinatie: ~b~"..destinationName,1,1,0.5,0.8,0.8,255,255,255,255)
							trucker_drawTxt("~g~Incarcatura: ~b~"..destinationType,1,1,0.5,0.84,0.8,255,255,255,255)
							trucker_drawTxt("~g~Distanta: ~r~"..(distanceLeft/1000).." ~b~KM ~w~| ~g~Viata Remorca: ~r~"..GetEntityHealth(theTrailer).." ~b~HP",4,1,0.5,0.87,0.5,255,255,255,255)
						end
					end
				else
					finishJob()
					vRP.notify({"~w~[TRUCK] ~r~Cursa a fost anulata!"})
...
CreateThread(function()
	while true do
		...
		if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 40.0)then
			...
			DrawText3D(x,y,z+0.15, "~g~Transport marfa de import", 1, 7)
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 5)then
				...
				if (incircle == true) then
					...
					if (hasTruck == 1) then
						if(inJob == false)then
							trucker_drawTxt("~y~[TRUCKER] ~g~Apasa ~r~[E] ~g~pentru a accesa lista curselor!",1,1,0.5,0.8,0.8,255,255,255,255)
						else
							trucker_drawTxt("~y~[TRUCKER] ~r~Ai deja o cursa activa!",1,1,0.5,0.74,0.8,255,255,255,255)
						end	
						if(IsControlJustReleased(1, 51)) and (inJob == false)then
							TriggerServerEvent('openTruckerJobs')
						end
					else
						trucker_drawTxt("~y~[TRUCKER] ~r~Ai nevoie de un tir personal!",1,1,0.5,0.8,0.8,255,255,255,255)
					end
				end
