...
AddEventHandler("buyCan", function()
	...
	GiveWeaponToPed(GetPlayerPed(-1), 0x34A67B97, 999,  0, true)
end)
...
function DisplayHud()
		--if(IsPedInAnyBoat(GetPlayerPed(-1)) or IsPedInAnyHeli(GetPlayerPed(-1)) or IsPedInAnyPlane(GetPlayerPed(-1)) or IsPedInAnyVehicle(GetPlayerPed(-1), false))then 
		--	local theVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
		--	if(theVehicle)then
