...
function startExercise(animInfo, pos)
    ...
    Citizen.CreateThread(function()
        while exercising do
            ...
            DrawText2D(0.505, 0.975, 1.0,1.0,0.33, "APASA ~r~[DELETE]~w~ PENTRU A OPRII ANTRENAMENTUL", 255, 255, 255, 255)
        end
    end)
        ...
        while canExercise do
            ...
            if procent <= 99 then
                ...
                if IsControlJustPressed(0, Keys["SPACE"]) then -- press space to exit training
					...
					TriggerServerEvent('vrp_sala:exerciseGym', 0.1)
                    canExercise = true
                end
                ...
                ExitTraining(animInfo["exitDict"], animInfo["exitAnim"], animInfo["exitTime"])
				TriggerServerEvent('vrp_sala:exerciseGym', 0.2)
...
AddEventHandler("vrp_sala:Corrida", function()
    ...
    Citizen.CreateThread(function()
        while doingMotion do
            ...
            DrawText2D(0.505, 0.95, 1.0,1.0,0.4, "~b~PRONTAJ:~w~ " .. tonumber(string.format("%.1f", motionProcent/1000)) .. "%", 255, 255, 255, 255)
            if motionProcent >= 100000 then
				TriggerServerEvent('vrp_sala:exerciseRunning', motionProcent/10000)
                doingMotion = false
                motionProcent = 0
				TriggerEvent('chatMessage', '', {255, 0, 0}, "Opreste-te", {255, 255, 255, 1.0,'', 0, 0, 100, 0.5})
            end
        end
    end)
    ...
    if doingMotion then
        ...
        if motionTimesDone <= 2 then
			TriggerEvent('chatMessage', '', {255, 0, 0}, "Ai inceput antrenamentul", {255, 255, 255, 1.0,'', 0, 0, 100, 0.5})
            print(motionTimesDone)
        else
			TriggerEvent('chatMessage', '', {255, 0, 0}, "Ai obosit", {255, 255, 255, 1.0,'', 100, 100, 0, 0.5})
            doingMotion = false
        end
    else
		TriggerEvent('chatMessage', '', {255, 0, 0}, "Te-ai oprit.", {255, 255, 255, 1.0,'', 0, 0, 100, 0.5})
		TriggerServerEvent('vrp_sala:exerciseRunning', motionProcent/10000)
    end
end)
