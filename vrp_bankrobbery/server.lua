...
local bankBox = {
	[1] = {name = "Diamonds", reward = 80000, from = 1, to = 4},
	[2] = {name = "Gold Bars", reward = 60000, from = 2, to = 5},
	[3] = {name = "Documents", reward = 10000, from = 5, to = 8},
	[4] = {name = "Jewelry", reward = 40000, from = 3, to = 10},
	[5] = {name = "Paintings", reward = 25000, from = 1, to = 3}
}
...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		cops = vRP.getUsersByGroup({"Cadet"})
		sias = vRP.getUsersByGroup({"Agent S.I.A.S"})
		armata = vRP.getUsersByGroup({"Militar"})
		sri = vRP.getUsersByGroup({"Agent S.R.I"})
		for i, v in pairs(cops) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Jaful a fost anulat la Banca: ^2" .. banks[robb].nameofbank)
		end	
	    for i, v in pairs(sias) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Jaful a fost anulat la Banca: ^2" .. banks[robb].nameofbank)
		end	
	    for i, v in pairs(armata) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Jaful a fost anulat la Banca: ^2" .. banks[robb].nameofbank)
		end	
	   for i, v in pairs(sri) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Jaful a fost anulat la Banca: ^2" .. banks[robb].nameofbank)			
		end
	end
...
AddEventHandler('playerDropped', function(robb)
	if(robbers[source])then
		...
		for i, v in pairs(cops) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Suspectul s-a Deconectat de pe Server și Jaful Bancar a fost anulat !")
		end	
		for i, v in pairs(sias) do
			local thePlayer = vRP.getUserSource({v})
			TriggerClientEvent('chatMessage', thePlayer, '(SYSTEM)', {255, 0, 0}, "Suspectul s-a Deconectat de pe Server și Jaful Bancar a fost anulat !")
...
local money_grab = {
	{"anim@heists@money_grab@duffel","enter",1},
	{"anim@heists@money_grab@duffel","enter_bag",1},
	{"anim@heists@money_grab@duffel","loop",1}
  }
...
AddEventHandler('es_bank:rob', function(robb)
	...
	if (os.time() - bank.lastrobbed) < 1200 and bank.lastrobbed ~= 0 then
		TriggerClientEvent('chatMessage', player, '(SYSTEM)', {255, 0, 0}, "Această Bancă a fost deja Jefuită. Așteptați: ^3" .. (2400 - (os.time() - bank.lastrobbed)) .. "^0 secunde.")
		return
	end
		...
		armata = vRP.getUsersByGroup({"Militar"})
		sri = vRP.getUsersByGroup({"Agent S.R.I"})
	if (vRP.hasGroup({user_id,"cop"}) or vRP.hasGroup({user_id,"Paramedic"}) or vRP.hasGroup({user_id,"Agent S.R.I"}) or vRP.hasGroup({user_id,"Agent S.I.A.S"}) or vRP.hasGroup({user_id,"Militar Armata"})) then
		vRPclient.notify(player,{"~r~Factiuniile Guvernamentale nu pot Jefuii Banci."})
		return		
	else
		local enforcers = tonumber(#cops)
		if (enforcers >= 2) then
			if banks[robb] then
				...
				for i, v in pairs(cops) do
						local thePlayer = vRP.getUserSource({v})
						TriggerClientEvent('chatMessage', thePlayer, '(ServerV2-ls NEWS)', {255, 0, 0}, "Un Jaf Bancar este în desfășurare la Banca: ^2" .. bank.nameofbank)
				end		
				for i, v in pairs(sias) do
						local thePlayer = vRP.getUserSource({v})
						TriggerClientEvent('chatMessage', thePlayer, '(ServerV2-ls NEWS)', {255, 0, 0}, "Un Jaf Bancar este în desfășurare la Banca: ^2" .. bank.nameofbank)
				...
				for i, v in pairs(armata) do
					...
					TriggerClientEvent('chatMessage', player, '(SYSTEM)', {255, 0, 0}, "Ai pornit un Jaf Bancar la Banca: ^2" .. bank.nameofbank .. "^0, nu te îndepărta de Bancă !")
					TriggerClientEvent('chatMessage', player, '(SYSTEM)', {255, 0, 0}, "Ține-te bine timp de:^1 10 ^0minute pentru a fura Bunuriile.")
					TriggerClientEvent('chatMessage', -1, '(ServerV2-ls NEWS)', {255, 0, 0}, "^4Poliția Română ^0avertizează ^3Civilii ^0să nu se apropie de Banca: ^2" .. bank.nameofbank)
					robb = robb
					...
					SetTimeout(480000, function()
						if(user_id)then
							if (robbers[player] ~= nil) then								
								...
								local theBlip = vRPclient.addBlip(player,{x,y,z,431,49,"Vinde Bunuriile"}, function(id)
								...
								local function launder_enter(source,area)
									...
									if(robbers[source])then
										...
										vRPclient.notify(source,{"~w~(SYSTEM) ~g~Ai primit: ~r~$"..totalMoney.." ~g~Bani!"})
										vRP.giveInventoryItem({user_id, "money", totalMoney, false})	
										robbers[source] = nil
								...
								TriggerClientEvent('chatMessage', player, '(SYSTEM)', {255, 0, 0}, "^0Du-te la ^1Dolarul Roșu ^0pentru a vinde OBIECTELE FURATE.")
								
								for i, v in pairs(cops) do
									local thePlayer = vRP.getUserSource({v})
									TriggerClientEvent('chatMessage', thePlayer, '(ServerV2-ls NEWS)', {255, 0, 0}, "S-a terminat Jaful Bancar la : ^2" .. bank.nameofbank .. "^0 ! Prinde-ți fugarul înainte de a scăpa cu Bunuriile.")
								end
							else 
								return
							end
					...
					vRPclient.notify(player,{"~r~Spargi deja o altă Bancă."})
				end	
			end
		else
